...
test("should render landing page", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("landing.png", { fullPage: true, maxDiffPixelRatio: 0.8 });

    await themeToggle.click();
    await expect(page).toHaveScreenshot("landing-dark.png", { fullPage: true, maxDiffPixelRatio: 0.8 });
    await themeToggle.click();
});
...
test("should render privacy policy page", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("privacy-policy.png", { fullPage: true, maxDiffPixelRatio: 0.8 });
});
