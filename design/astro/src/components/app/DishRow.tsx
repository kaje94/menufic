...
/* ============================================================
   ...
   token — var(--color-*), var(--radius-field). Documented carve-outs
   inherited verbatim from the source:
     ...
     - `.grip` border-radius: 8px — a small utilitarian radius transcribed
       verbatim from the source; not one of the daisyUI --radius-* tokens
...
export default function DishRow({ d }: { d: Dish }) {
  // Speech-friendly name for aria-labels only: source spells "&" as "and" so
  // screen readers don't announce the literal ampersand glyph (source
  // menu-editor.html:669,676,682,683 — e.g. "Sourdough and cultured butter").
  // The VISIBLE `.it-name` text keeps the real `d.name` (with "&").
  const speakName = d.name.replace(/&/g, 'and');
  return (
    <article className="item" data-status={d.availability}>
        <button className="grip" type="button" aria-label={`Drag to reorder ${speakName}`}>
          <GripIcon />
        </button>
        ...
        <div className="seg" role="radiogroup" aria-label={`Availability of ${speakName}`}>
          {STATUSES.map((s) => (
            <button
              key={s.val}
              type="button"
        ...
        <div className="it-actions">
          <button className="rbtn" type="button" aria-label={`Edit ${speakName}`}>
          ...
          <button className="rbtn del" type="button" aria-label={`Delete ${speakName}`}>
